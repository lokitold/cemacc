...
//require_once(APPLICATION_PATH.'/../library/Core/Jasper/Constants.php');
...
class Core_Jasper_Client {
	...
	public function __construct($hostname = 'localhost', $port = '8080', $username = null, $password = null, $baseUrl = "/jasperserver-pro", $orgId = null)
	{
		...
		$this->restReq = new \Core_Jasper_RESTRequest(); // This object is recycled
                if (!empty($this->orgId)) {
	 ...
	 */
	public function getAttributes(Core_Jasper_User $user) {
		$result = array();
...
//		if ($data = $this->prepAndSend($url, array(200), 'GET', null, true)
		...
		foreach ($xml->Item as $item) {
			$tempAttribute = new Core_Jasper_Attribute(
				$item->name,
				$item->value);
	 ...
	 */
	public function postAttributes(Core_Jasper_User $user, $attributes) {
		$url = $this->restUrl . ATTRIBUTE_BASE_URL . '/' . $user->getUsername();
		if ($user->getTenantId() !== null) {
			$url .= PIPE . $user->getTenantId();
		}
	...
	public function getUsers($searchTerm = null) {
		...
		foreach ($xml->user as $user) {
			...
			foreach ($user->roles as $role) {
				$tempUser->addRole(Core_Jasper_Client::roleToRoleObj($role));
			}
			$result[] = $tempUser;
		}
     ...
     * @return bool - based on success of function
     */
	public function postUser(Core_Jasper_User $user) {
	 ...
	 * @return bool - based on success of function
	 */
	public function deleteUser(Core_Jasper_User $user) {
		$url = $this->restUrl . USER_BASE_URL . '/' . $user->getUsername();
	...
	public function getOrganization($org, $listSub = false) {
		...
		if($data = $this->prepAndSend($url, array(200), 'GET', null, true)) {
			...
			$orgObj = new Core_Jasper_Organization(
				$xml->alias,
				$xml->id,
				$xml->parentId,
				$xml->tenantName,
	 ...
	 */
	public function putOrganization(Core_Jasper_Organization $org) {
	 ...
	 */
	public function deleteOrganization(Core_Jasper_Organization $org) {
		$url = $this->restUrl . ORGANIZATION_BASE_URL . '/' . $org;
		if($this->prepAndSend($url, array(200), 'DELETE')) {
			return true;
		}
	 ...
	 * @throws Exception - if HTTP request doesn't succeed
	 */
	public function postOrganization(Core_Jasper_Organization $org) {
	...
	public function getRoles($searchTerm = null, $tenantId = null) {
		...
		foreach ($xml->role as $role) {
			$tempRole = new Core_Jasper_Role($role->roleName,
					$role->tenantId,
	 ...
	 * @throws Exception - if http request doesn't succeed
	 */
	public function putRole(Core_Jasper_Role $role) {
     ...
     * @return bool - based on success of function
     */
	public function deleteRole(Core_Jasper_Role $role) {
	 ...
	 */
	public function postRole(Core_Jasper_Role $role, $oldName = null) {
	...
	public function getRepository($uri = null, $query = null, $wsType = null, $recursive = null, $limit = null) {
		...
		foreach ($xml->resourceDescriptor as $rd) {
			$obj = Core_Jasper_ResourceDescriptor::createFromXML($rd->asXML());
			$result[] = $obj;
		}
		return $result;
	}
	...
	public function getResource($path, $fileData = false, $ic_get_query_data = null, $p = null, $pl = null) {
		...
		return Core_Jasper_ResourceDescriptor::createFromXML($data);
		}
	}
     ...
     * @return bool
     */
	public function putResource($path, Core_Jasper_ResourceDescriptor $rd, $file = null) {
		$url = $this->restUrl . '/resource' . $path;
		$statusCode = null;
		if (!empty($file)) {
     ...
     * @return bool - based on success of function
     */
	public function postResource($path, Core_Jasper_ResourceDescriptor $rd, $file = null) {
		$url = $this->restUrl . '/resource' . $path;
		$statusCode = null;
		if (!empty($file)) {
	...
	public function getJobs($query = null, $searchByName = false) {
		...
		if (!empty($data)) {
			...
			foreach($xml->jobsummary as $job) {
				$result[] = new Core_Jasper_JobSummary($job->id, $job->label, $job->reportUnitURI, $job->version, $job->owner,
	...
	public function getJob($id) {
		...
		$data = $this->prepAndSend($url, array(200), 'GET', null, true);
		$result = Core_Jasper_Job::createFromXML($data);
		return $result;
	}
	 ...
	 */
	public function putJob(Core_Jasper_Job $job) {
		$url = $this->restUrl2 . '/jobs';
	 ...
	 * @return bool - based on success of function
	 */
	public function postJob(Core_Jasper_Job $job) {
	...
	public function getPermissions($uri) {
		...
		$data = $this->prepAndSend($url, array(200), 'GET', null, true);
		return Core_Jasper_Permission::createFromXML($data);
	}
	...
	public function updatePermissions($uri, $permissions) {
		$url = $this->restUrl . '/permission' . $uri;
		$body = Core_Jasper_Permission::createXMLFromArray($permissions);
     ...
     * @return bool - based on success of function
     */
	public function deletePermission(Core_Jasper_Permission $perm) {
		...
		} elseif ($recipient instanceof Core_Jasper_Role) {
	...
	public function getReportOptions($uri) {
		...
		return Core_Jasper_ReportOptions::createFromJSON($data);
	}
	...
	public function getReportInputControls($uri) {
		...
		$data = $this->prepAndSend($url, array(200), 'GET', null, true, 'application/json', 'application/json');
		return Core_Jasper_InputOptions::createFromJSON($data);
	}
    ...
    public function getReportInputControlStructure($uri) {
        ...
        $data = $this->prepAndSend($url, array(200), 'GET', null, true, 'application/json', 'application/json');
        return Core_Jasper_InputStructure::createFromJSON($data);
    }

...
class Core_Jasper_ResourceDescriptor {
	...
	public static function createFromXML($xml) {
		...
		foreach($sxi->resourceDescriptor as $nestRd) {
			$temp->children[] = Core_Jasper_ResourceDescriptor::createFromXML($nestRd->asXML());
		}

		foreach($sxi->resourceProperty as $prop) {
			$temp->properties[] = Core_Jasper_ResourceProperty::createFromXML($prop->asXML());
		}
		return $temp;
	}
	...
	private function newXML() {
		...
		if(count($this->properties) > 0) {
			foreach($this->properties as $property) {
				Core_Jasper_ResourceDescriptor::sxml_append($result, $property->newXML());
			}
		}
		return $result;
	}
	...
	public function addProperty(Core_Jasper_ResourceProperty $rp) {
		$this->properties[] = $rp;
	}

	public function delProperty(Core_Jasper_ResourceProperty $rp) {

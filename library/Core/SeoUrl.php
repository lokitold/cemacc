...
class Core_SeoUrl extends Zend_Filter_Alnum
{
    ...
    public function filter($value, $valuechange='', $min ='',$ene=1)
    {
    	...
    	if(trim($valuechange)!=""){
    		$str = preg_replace('/\s\s+/',' ',preg_replace($pattern,' ',$value));
                if($ene==1){
                    ...
                    $replace=array("á","à","é","è","í","ì","ó","ò","ú","ù","Á","À","É","È","Í","Ì","Ó","Ò","Ú","Ù",".");
                    $change=array("a","a","e","e","i","i","o","o","u","u","A","A","E","E","I","I","O","O","U","U",".");                    
                } 
    		$str=trim(str_replace($replace,$change,$str));
                if(substr($str, -1)==$valuechange){
                    $str = substr($str, 0, -1);
                }
                if($min == 1)
    ...
    public function urlFriendly($value, $valuechange='', $min ='',$ene=1)
    {
